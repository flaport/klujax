...
def _xla_coo_vec_operation_f64(c, Ax, Ai, Aj, b, name):
...
def _xla_coo_vec_operation_c128(c, Ax, Ai, Aj, b, name):
    if isinstance(name, str):
        name = name.encode()
    Ax_shape = c.get_shape(Ax)
    Ai_shape = c.get_shape(Ai)
...
def solve_f64_xla_translation(c, Ax, Ai, Aj, b):
    return _xla_coo_vec_operation_f64(c, Ax, Ai, Aj, b, "solve_f64")
